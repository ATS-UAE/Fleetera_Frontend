...
function buildVehicleTracks(vehicle, vehicleIndex, depotGeofences) {
  ...
  for (let t = 0; t < numTrips; t++) {
    ...
    if (depot) { lat = depot.center.lat; lng = depot.center.lng; locationName = depot.name; }
...
// `geofences` as arguments (rather than importing them from ./index.js) so this
// module has no dependency on index.js — index.js depends on this one, not the
// other way round, avoiding a circular-import initialization-order error.
export function buildVehicleTrackData(vehicles, geofences) {
...
export function getTrackDistanceKm(track) {
